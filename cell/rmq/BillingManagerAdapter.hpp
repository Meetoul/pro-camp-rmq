  ...
  public:
    ...
    void activeRMQ()
    {
        ...
        channel_.declareExchange("biller", AMQP::topic).onSuccess([]() {
            std::cout << "declared biller exchange " << std::endl;
        });

        channel_.declareQueue("biller").onSuccess([](const std::string &name, uint32_t messagecount, uint32_t consumercount) {
            std::cout << "declared bilelr queue " << name << std::endl;
        });

        channel_.bindQueue("biller", "biller", "biller").onSuccess([]() {
            std::cout << "binded to biller queue " << std::endl;
        });

        channel_.consume("biller", AMQP::noack).onReceived([this](const AMQP::Message &message, uint64_t deliveryTag, bool redelivered) {
